...
function PivotTable(element, props) {
        ...
        if ($(this).find('th')[0].attributes[0].name === 'rowspan') {
          ...
          for (
            ...
            tds.each(function (tdIndex) {
              ...
              delete sum[tdIndex];
              sum[tdIndex] =parseInt(previous, 10) + parseInt(tds[tdIndex].innerHTML, 10);
            });
            // console.log('tds[0]', tds[0]);
            rowsArray.push({
              'startIndex': index,
              'value':  tds[0] ? parseInt(tds[0].innerHTML, 10) : '',
              'rowValue': `<tr>${$(allRows[index + rowSpanIndex])[0].innerHTML}</tr>`
            });
          }
  ...
  // on header click get column index to sort
