      ...
      if ($(this).find('th')[0].attributes[0]) {
        if ($(this).find('th')[0].attributes[0].name === 'rowspan') {
          ...
          for (
            ...
            if(rowSpanIndex === 0) {
              newTHead = `<th rowspan="${rowSpanCount + 1}">${$(allRows[index + rowSpanIndex]).find("th[rowspan]")[0].innerHTML}</th>`;
              const removedSpan = $(allRows[index + rowSpanIndex]).children('th').eq(0).remove();
              rowsArray.push({
                'startIndex': index,
          ...
          for (const [key] in Object.entries(addition)) {
          ...
          allRowsWithSum.push({ sum: addition, rowData: rows, subTotal:  subTotalRowData, sortedArray, newTHead});
...
function replaceAfterSort() {
  let rows = '';
  allRowsWithSum.forEach((element, index)=> {
